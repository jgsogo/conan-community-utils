...
def get_version():
    file_with_version = os.path.join(here, 'conan_community_tools', '__init__.py')
    with codecs.open(file_with_version, "rt") as f:
        return re.search("__version__ = '([0-9a-z.]+)'", f.read()).group(1)


setup(
    name='conan_community_tools',
    version=get_version(),
    long_description=get_long_description(),
    description="Utilities to work with a Conan community",
    url='https://github.com/jgsogo/conan-community-tools',
    author='jgsogo',
    ...
    entry_points={
        'console_scripts': [
            'conan_community_web=conan_community_tools.scripts.web:main',
        ],
    },
)
